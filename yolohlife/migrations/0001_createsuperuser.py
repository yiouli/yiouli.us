...
def createsuperuser(apps, schema_editor):
    ...
    admin_password = client.access_secret_version(name=name).payload.data.decode("UTF-8")  # type: ignore

    # Create a new user using acquired password
    from django.contrib.auth.models import User
    User.objects.create_superuser("admin", password=admin_password)  # type: ignore
